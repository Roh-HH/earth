...
<script>
	function popupOpen(){
		var popUrl = "/earth/main/messageWriteForm.et?uri=/earth/main/messageSendList.et"
		var popOption = "width=500, height=340, resizable=no, scrollbars=no, status=no;";
		window.open(popUrl,"",popOption);
	}
	function msgContent(messagenum){
		var popUrl = "/earth/main/messageContent.et?pageNum=${pageNum}&messagenum=" + messagenum;
		var popOption = "width=500, height=340, resizable=no, scrollbars=no, status=no;";
		window.open(popUrl,"",popOption);
	}
					...
					<div class="col-lg-8 posts-list">
						<div class="single-post">
							<div align="center">
								...
								<h1 class="entry-title">보낸 쪽지함</h1>
									<c:if test="${count == 0}">
										<table>
											<tr>
										...
										<table >
											...
											<c:forEach var="message" items="${messageList}">
												<tr>
													...
													<td>
														<button type="button" class="delete_btn" onclick="window.location='/earth/main/messageDelete.et?pageNum=${pageNum}&messagenum=${message.messagenum}&uri=/main/messageSendList.et#anc'">삭제</button>
													</td>
												</tr>
											</c:forEach>
										</table>
											...
											<nav class="pagination" style="float:center;">					
												<c:if test="${count > 0}">
													...
													<c:if test="${sel == null || search == null}">
														...
														<c:if test="${endPage < pageCount}">
															<a class="next page-numbers" href="/earth/main/messageSendList.et?pageNum=${startPage+pageBlock}#anc" class="pageNums">Next»</a>
														</c:if>
													</c:if>
												</c:if> <%-- end:count > 0 --%>
											</nav>
