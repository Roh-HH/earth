												...
												<c:if test="${count > 0}">
													...
													<c:if test="${sel != null && search != null}">
														<c:if test="${startPage > pageBlock}">
															<a class="back page-numbers" href="/earth/main/messageRemindList.et?pageNum=${startPage-pageBlock}&sel=${sel}&search=${search}#anc" class="pageNums">Back</a>
														</c:if>
														<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
															<c:choose>
																<c:when test="${pageNum eq i}">
																	<a class="current" href="/earth/main/messageRemindList.et?pageNum=${pageNum}&sel=${sel}&search=${search}#anc">${i}</a>
																</c:when>
																<c:otherwise>
																	<a class="page-numbers" href="/earth/main/messageRemindList.et?pageNum=${i}&sel=${sel}&search=${search}#anc" class="pageNums">${i}</a>
																</c:otherwise>
															</c:choose>
														</c:forEach>
														<c:if test="${endPage < pageCount}">
															<a class="next page-numbers" href="/earth/main/messageRemindList.et?pageNum=${startPage+pageBlock}&sel=${sel}&search=${search}#anc" class="pageNums">Next</a>
														</c:if>
													</c:if>
													...
													<c:if test="${sel == null || search == null}">
														<c:if test="${startPage > pageBlock}">
															<a class="back page-numbers" href="/earth/main/messageRemindList.et?pageNum=${startPage-pageBlock}#anc" class="pageNums">«Back</a>
														</c:if>
														<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
															<c:choose>
																<c:when test="${pageNum eq i}">
																	<a class="current" href="/earth/main/messageRemindList.et?pageNum=${pageNum}#anc">${i}</a>
																</c:when>
																<c:otherwise>
																	<a class="page-numbers" href="/earth/main/messageRemindList.et?pageNum=${i}#anc" class="pageNums">${i}</a>
																</c:otherwise>
															</c:choose>
														</c:forEach>
														<c:if test="${endPage < pageCount}">
