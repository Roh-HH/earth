...
<script>
	...
	function msgContent(messagenum){
		var popUrl = "/earth/main/messageContent.et?pageNum=${pageNum}&uri=/earth/main/messageReceiveList.et&messagenum=" + messagenum;
		var popOption = "width=500, height=340, resizable=no, scrollbars=no, status=no;";
		window.open(popUrl,"",popOption);
	}
					...
					<div class="col-lg-8 posts-list">
						<div class="single-post">
							<div align="center">
								...
								<h1 class="entry-title">받은 쪽지함</h1>
									<c:if test="${count == 0}">
										<table>
											<tr>
									...
									<c:if test="${count != 0}">
										<table >
											...
											<c:forEach var="message" items="${messageList}">
												<tr>
													...
													<td>
														<button onclick="window.location='/earth/main/messageRemind.et?pageNum=${pageNum}&messagenum=${message.messagenum}&uri=/main/messageReceiveList.et'"
																style="background-color:#1E88E5; border:none; color:#ffffff">쪽지 보관</button>
										...
										<div align="center" class="col-12 text-center">
												...
												<c:if test="${count > 0}">
													...
													<c:if test="${sel != null && search != null}">
														<c:if test="${startPage > pageBlock}">
															<a class="back page-numbers" href="/earth/main/messageReceiveList.et?pageNum=${startPage-pageBlock}&sel=${sel}&search=${search}#anc" class="pageNums">Back</a>
														</c:if>
														<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
															<c:choose>
																<c:when test="${pageNum eq i}">
																	<a class="current" href="/earth/main/messageReceiveList.et?pageNum=${pageNum}&sel=${sel}&search=${search}#anc">${i}</a>
																</c:when>
																<c:otherwise>
																	<a class="page-numbers" href="/earth/main/messageReceiveList.et?pageNum=${i}&sel=${sel}&search=${search}#anc" class="pageNums">${i}</a>
																</c:otherwise>
															</c:choose>
														</c:forEach>
														<c:if test="${endPage < pageCount}">
															<a class="next page-numbers" href="/earth/main/messageReceiveList.et?pageNum=${startPage+pageBlock}&sel=${sel}&search=${search}#anc" class="pageNums">Next</a>
														</c:if>
													</c:if>
													...
													<c:if test="${sel == null || search == null}">
														<c:if test="${startPage > pageBlock}">
															<a class="back page-numbers" href="/earth/main/messageReceiveList.et?pageNum=${startPage-pageBlock}#anc" class="pageNums">«Back</a>
														</c:if>
														<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
															<c:choose>
																<c:when test="${pageNum eq i}">
																	<a class="current" href="/earth/main/messageReceiveList.et?pageNum=${pageNum}#anc">${i}</a>
																</c:when>
																<c:otherwise>
																	<a class="page-numbers" href="/earth/main/messageReceiveList.et?pageNum=${i}#anc" class="pageNums">${i}</a>
																</c:otherwise>
															</c:choose>
														</c:forEach>
														<c:if test="${endPage < pageCount}">
											...
											<form action="/earth/main/messageReceiveList.et" >
