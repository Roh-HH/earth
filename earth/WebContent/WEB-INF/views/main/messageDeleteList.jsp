...
<script>
	function msgSend(){
		var popUrl = "/earth/main/messageWriteForm.et"
		var popOption = "width=500, height=340, resizable=no, scrollbars=no, status=no;";
		window.open(popUrl,"",popOption);
	}
	function msgContent(messagenum){
		var popUrl = "/earth/main/messageContent.et?pageNum=${pageNum}&uri=/earth/main/messageDeleteList.et&messagenum=" + messagenum;
		var popOption = "width=500, height=340, resizable=no, scrollbars=no, status=no;";
		window.open(popUrl,"",popOption);
	}
				...
				<section class="right_side" style="width:600px;">        
					<div class="col-lg-8 posts-list">
						<div class="single-post">
							<div align="center">
			                    <div class="blog_details" id="anc" style="width:880px; height:900px;">
					        	<h3 class="page-title">쪽지</h3>
								<h1 class="entry-title">휴지통</h1>
									<c:if test="${count == 0}">
										<table>
											<tr>
										...
										<table >
											...
											<c:forEach var="message" items="${messageList}">
												<tr>
													...
													<td>
														<button onclick="window.location='/earth/main/messageRevive.et?pageNum=${pageNum}&messagenum=${message.messagenum}#anc'"
																style="background-color:#1E88E5; border:none; color:#ffffff">쪽지 복구</button>
												...
												<c:if test="${count > 0}">
													...
													<c:if test="${sel != null && search != null}">
														<c:if test="${startPage > pageBlock}">
															<a class="back page-numbers" href="/earth/main/messageDeleteList.et?pageNum=${startPage-pageBlock}&sel=${sel}&search=${search}#anc" class="pageNums">Back</a>
														</c:if>
														<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
															<c:choose>
																<c:when test="${pageNum eq i}">
																	<a class="current" href="/earth/main/messageDeleteList.et?pageNum=${pageNum}&sel=${sel}&search=${search}#anc">${i}</a>
																</c:when>
																<c:otherwise>
																	<a class="page-numbers" href="/earth/main/messageDeleteList.et?pageNum=${i}&sel=${sel}&search=${search}#anc" class="pageNums">${i}</a>
																</c:otherwise>
															</c:choose>
														</c:forEach>
														<c:if test="${endPage < pageCount}">
															<a class="next page-numbers" href="/earth/main/messageDeleteList.et?pageNum=${startPage+pageBlock}&sel=${sel}&search=${search}#anc" class="pageNums">Next</a>
														</c:if>
													</c:if>
													...
													<c:if test="${sel == null || search == null}">
														<c:if test="${startPage > pageBlock}">
															<a class="back page-numbers" href="/earth/main/messageDeleteList.et?pageNum=${startPage-pageBlock}#anc" class="pageNums">«Back</a>
														</c:if>
														<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
															<c:choose>
																<c:when test="${pageNum eq i}">
																	<a class="current" href="/earth/main/messageDeleteList.et?pageNum=${pageNum}#anc">${i}</a>
																</c:when>
																<c:otherwise>
																	<a class="page-numbers" href="/earth/main/messageDeleteList.et?pageNum=${i}#anc" class="pageNums">${i}</a>
																</c:otherwise>
															</c:choose>
														</c:forEach>
														<c:if test="${endPage < pageCount}">
															<a class="next page-numbers" href="/earth/main/messageDeleteList.et?pageNum=${startPage+pageBlock}#anc" class="pageNums">Next»</a>
														</c:if>
													</c:if>
												</c:if> <%-- end:count > 0 --%>
											</nav>
											<input type="button" value="쪽지함 비우기" onclick="window.location='/earth/main/messageClearing.et#anc'"/>
											<br/><br/>
											<form action="/earth/main/messageDeleteList.et" >
